<footer class="overflow-hidden transition-[padding-left] ease-in-out duration-300 mt-auto flex-0 relative pb-[40px]"
	...
	<img src="{{ asset('images/bg-footer.png') }}"
	...
	<div class="z-[2] flex flex-col lg:flex-row gap-y-8 gap-x-20 justify-between text-white py-12 md:pb-[100px] px-6 relative max-w-[1536px] mx-auto">
		<a class="text-sm leading-6 " href="{{ get_current_language_code() == 'fr' ? '/' : '/en' }}">
			@if(get_field('logo_light', 'option'))
				<img decoding="async" loading="lazy" width="250"
					 src="{{ get_field('logo_light', 'option') }}"
	...
	<div class="flex flex-col md:flex-row z-[2] md:z-[99999] -mt-[1rem] pb-[50px] md:py-2 md:mt-0 gap-y-2 relative md:fixed bottom-0 w-full left-0 justify-between md:items-center md:bg-foreground-gray px-6">
